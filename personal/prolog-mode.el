...
;;
;; Version: 1.43 (modified by Thierry.Martinez@inria.fr for better
;; support of different Prolog implementations, especially GNU Prolog)
;;
;; Used to be prolog.el version 1.41 (for SICStus Prolog 3.12.1), from 0.1.31 (FSF)
;;
...
;; [PM]
(defconst prolog-mode-version '(1 . 43)
...
swi     - SWI Prolog
jekejeke - Jekejeke Prolog
gprolog - GNU Prolog")
(make-variable-buffer-local 'prolog-system)

(defvar prolog-session nil
  "Underlying prolog process name")
(make-variable-buffer-local 'prolog-session)

(defvar prolog-preferred-by-file nil
  "Associative list between file names and pairs (system, session name).")
...
(defvar prolog-system-version
  '((sicstus  (eval (if (boundp 'sicstus-version)
                        sicstus-version
                      '(3 . 7))))       ; [PM] this should not change, sicstus-version is set in 3.8 and later
    (swi      (0 . 0))
    (jekejeke (0 . 0))
    (mercury  (0 . 0))
    (eclipse  (3 . 7))
...
(defvar prolog-known-systems '(eclipse mercury sicstus swi gprolog))
...
(defun prolog-system-name (system)
  (cond
   ((eq system 'eclipse) "ECLiPSe")
   ((eq system 'mercury) "Mercury")
   ((eq system 'sicstus) "SICStus")
   ((eq system 'swi) "SWI Prolog")
   ((eq system 'jekejeke) "Jekejeke Prolog")
   ((eq system 'gprolog) "GNU Prolog")
   (t "")))

;; [thierry.martinez@inria.fr] Extracted from prolog-mode-variables
(defun prolog-process-mode-name (basename)
  (concat basename " [" (prolog-system-name prolog-system)
          (if prolog-session
              (concat " <" prolog-session ">")
            nil) "]"))

(defun prolog-process-pretty-mode-name (basename)
  (append
   (list basename)
   (list " [")
   (list
    (let ((map (make-sparse-keymap)))
      (define-key map [mode-line mouse-1] 'set-prolog-system)
      (list ':propertize
            (list (prolog-system-name prolog-system))
            'keymap map
            'mouse-face 'highlight
            'help-echo "Set Prolog system")))
   (if prolog-session
       (append
        (list " <")
        (list
         (let ((map (make-sparse-keymap)))
           (define-key map [mode-line mouse-1] 'set-prolog-session)
           (list ':propertize
                 (list prolog-session)
                 'face 'italic
                 'keymap map
                 'mouse-face 'highlight
                 'help-echo "Set Prolog session")))
        (list ">"))
      nil)
   (list "]")))

(defun update-prolog-process ()
  "Initialize Prolog process specific variables"
  (interactive)
  (if (eq major-mode 'prolog-mode)
      (setq mode-name (prolog-process-pretty-mode-name "Prolog")))
  (let ((vars '(prolog-keywords prolog-types prolog-mode-specificators
    ...
    (while vars
      ;; [thierry.martinez@inria.fr] added call to make-local-variable
      (set (make-local-variable (intern (concat (symbol-name (car vars)) "-i")))
...
;      (setq compile-command prolog-compile-string-i))
	...
	(setq compile-command prolog-compile-string-i)))
  (prolog-load-system-support prolog-system))

(defvar set-prolog-system-history nil)

;; [thierry.martinez@inria.fr]
(defun set-prolog-system (system)
  "Change Prolog system."
  (interactive
   (list
    (intern
     (completing-read "Prolog system: "
		      (mapcar 'symbol-name prolog-known-systems)
		      nil t nil 'set-prolog-system-history (symbol-name prolog-system) t))))
  (setq prolog-system system)
  (if (and prolog-session (not (eq system (prolog-session-system prolog-session))))
    (setq prolog-session nil))
  (prolog-store-preferred)
  (update-prolog-process))

(defvar ask-for-prolog-session-history nil)

(defun ask-for-prolog-session ()
  (let
      ((default-prolog-session
         (if prolog-session
             prolog-session
           (car (delq prolog-session (get-prolog-sessions prolog-system)))))
       (session nil))
    (if (not default-prolog-session)
        (setq default-prolog-session (symbol-name prolog-system)))
    (while
        (progn
          (setq session
                (completing-read "Prolog session name: "
                                 (get-prolog-sessions)
                                  nil nil session 'ask-for-prolog-session-history
                                 default-prolog-session))
          (not (prolog-switch-system-confirm (prolog-session-system session))))
      nil)
    session))

(defun get-prolog-sessions (&optional system)
  "List available Prolog sessions.  If `system' is not nil, restrict
to sessions whose Prolog system is `system'."
  (let ((prolog-sessions
         (delq nil
               (mapcar
                (lambda (buffer)
                  (let ((buffer-system-and-session
                         (save-excursion
                           (set-buffer buffer)
                           (if (eq major-mode 'prolog-inferior-mode)
                               (cons prolog-system prolog-session)
                             nil))))
                    (if (and buffer-system-and-session
                             (or (not system)
                                 (eq (car buffer-system-and-session) system)))
                        (cdr buffer-system-and-session)
                      nil)))
                  (buffer-list)))))
    (if system
        (add-to-list 'prolog-sessions (symbol-name system))
      (mapcar
       (lambda (system)
         (add-to-list 'prolog-sessions (symbol-name system)))
       prolog-known-systems))
    prolog-sessions))

(defun prolog-session-system (session)
  (if (memq (intern session) prolog-known-systems)
      (intern session)
    (let ((sessions
           (delq nil
                 (mapcar
                  (lambda (buffer)
                    (save-excursion
                      (set-buffer buffer)
                      (if (and (eq major-mode 'prolog-inferior-mode))
                          (cons prolog-session prolog-system)
                        nil)))
                    (buffer-list)))))
      (or (cdr (assoc session sessions)) prolog-system))))

(defun prolog-switch-system-confirm (system)
  (if (eq system prolog-system)
      t
    (if (y-or-n-p
         (concat
          "This will switch to `"
          (prolog-system-name system)
          "' prolog system, are you sure?"))
        (progn
          (set-prolog-system system)
          t)
      nil)))

(defun set-prolog-session (session)
  "Change Prolog session name."
  (interactive (list (ask-for-prolog-session)))
  (setq prolog-session session)
  (prolog-store-preferred)
  (update-prolog-process))

(defun prolog-store-preferred ()
  "Memorize the pair system/session for the file"
  (let ((preference (cons prolog-system prolog-session))
        (preferred (assoc buffer-file-name prolog-preferred-by-file)))
    (if preferred
        (setcdr preferred preference)
      (setq prolog-preferred-by-file
            (cons
             (cons buffer-file-name preference) prolog-preferred-by-file)))))

(defun prolog-restore-preferred ()
  "Restore the last pair system/session for the file"
  (let ((preferred (assoc buffer-file-name prolog-preferred-by-file)))
    (if preferred
        (progn
          (setq prolog-system (car (cdr preferred))
                prolog-session (cdr (cdr preferred)))
          (update-prolog-process)))))

(defun prolog-ensure-process-defined ()
  (if prolog-session
      (if
          (not
           (prolog-switch-system-confirm
            (prolog-session-system prolog-session)))
          (keyboard-quit))
    (set-prolog-session (ask-for-prolog-session))))
...
if that value is non-nil."
  ...
  ;; [thierry.martinez@inria.fr] Updates for prolog process
  (prolog-restore-preferred)
  (update-prolog-process)
  (add-hook 'hack-local-variables-hook 'update-prolog-process)
  (set
   (make-local-variable 'safe-local-variable-values)
   (append
    (mapcar (lambda (x) (cons 'prolog-system x)) prolog-known-systems)
...
;; [thierry.martinez@inria.fr] added system argument
(defun prolog-inferior-mode (&optional system session)
...
\\[comint-quit-subjob] sends quit signal, likewise."
  ...
  (cond ((not (eq major-mode 'prolog-inferior-mode))
	 ...
	 (comint-mode)
         (if system (setq prolog-system system))
         (if session (setq prolog-session session))
	 (setq comint-input-filter 'prolog-input-filter)
	 (setq major-mode 'prolog-inferior-mode)
         (setq mode-name (prolog-process-mode-name "Inferior Prolog"))
...
; [thierry.martinez@inria.fr]
(defun prolog-process-name ()
  (prolog-process-mode-name "prolog"))
(defun inferior-prolog-buffer-name ()
  (concat "*" (prolog-process-mode-name "prolog") "*"))
...
With prefix argument ARG, restart the Prolog process if running before."
  ...
  (prolog-system-override run-prolog (arg)
    (let ((process-name (prolog-process-name)))
      (if (and arg (get-process process-name))
          (progn
            (process-send-string process-name "halt.\n")
            (while (get-process process-name) (sit-for 0.1))))
      (let ((buff (buffer-name)))
        (if (not (string= buff (inferior-prolog-buffer-name)))
...
may still return the name of the Prolog process buffer if it exists."
  (prolog-system-override prolog-ensure-process (wait no-create)
    ...
    (prolog-ensure-process-defined)

    (let* ((process-name (prolog-process-name))
           (buf-name (inferior-prolog-buffer-name))) ; see make-comint docs
      (cond ((comint-check-proc buf-name)
            ...
            (no-create
             (and (get-buffer buf-name)
                  buf-name))
            (t
             (apply 'make-comint process-name prolog-program-name-i nil
                    prolog-program-switches-i)
             (let ((local-prolog-system prolog-system)
                   (local-prolog-session prolog-session))
               (save-excursion
                 (set-buffer buf-name)
                  ;; [thierry.martinez@inria.fr]
                  ;; Prolog inferior mode inherits from prolog-system
                  (prolog-inferior-mode local-prolog-system local-prolog-session)
                  (if wait
                      (progn
                        (goto-char (point-max))
                        (while
...
(defun prolog-process-insert-and-send-string (string &optional process)
  ...
  (if (null process) (setq process (prolog-process-name)))
...
If COMPILEP is non-nil then use compilation, otherwise consulting."
   ...
   (let ((tmpfile prolog-temp-filename)
     ...
     (process-send-string
      (prolog-process-name) (prolog-build-prolog-command
...
This function must be called from the source code buffer."
  ...
  (let* ((buffer (get-buffer-create prolog-compilation-buffer))
	 ...
	 (process (get-process (prolog-process-name)))
...
;       (prolog-build-prolog-command compilep file real-file first-line))
	...
	(sit-for 0.1)
	(if (not (get-process (prolog-process-name)))
...
Argument OUTPUT is a name of the output file."
  ...
  (let (outputtype)
	...
	(let (input)
	  ...
	  (process-send-string (prolog-process-name) input)
...
(defun prolog-help-online (predicate)
  (prolog-ensure-process)
  (process-send-string (prolog-process-name) (concat "help(" predicate ").\n"))
  (display-buffer (inferior-prolog-buffer-name)))
...
This function is only available when `prolog-system' is set to `swi'."
  ...
  (prolog-system-override prolog-help-apropos (string)
    (cond
     ((eq prolog-system 'swi)
      (prolog-ensure-process)
      (process-send-string (prolog-process-name) (concat "apropos(" string ").\n"))
      (display-buffer (inferior-prolog-buffer-name)))
     (t
...
(defun prolog-goto-prolog-process-buffer ()
  ...
  (switch-to-buffer-other-window (inferior-prolog-buffer-name))
  (goto-char (point-max)))
...
;; [thierry.martinez@inria.fr] added 'consp' case for Emacs 23
(defun prolog-build-case-strings-helper (char &optional ignore)
  ...
  (let* ((case-fold-search nil)          ; make char-equal the same as char=
    ...
    (if (consp char)
	(let ((counter (car char)))
	  (while (<= counter (cdr char))
	    (funcall apply-char counter)
	    (setq counter (1+ counter))))
      (funcall apply-char char))))
